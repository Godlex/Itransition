...
function lp_create_task_with_many_executors( sCommand, iPersonID, tePerson, SCOPE_WVARS, iTaskID, iExecutorID, iSourceObjectID )
{
	oRes = new Object();
	oRes.error = 0;
	oRes.errorText = "";
	...
	switch( sCommand )
	{
		...
		case "submit_form":
			...
			if( iTaskID == null )
			{
                ...
                oRes.taskID = taskDoc.TopElem.id;
			}
			else
			{
						...
						if ( fldCustomElems != null )
						{
							...
							if( ArrayOptFirstElem( fldCustomElemsFields ) != undefined )
							{
								for( _field in fldCustomElemsFields )
								{
									...
									{
										bShow_arrCustomFields = SafeEval( tools.create_filter_javascript( _field.view.conditions ), [ { 'fldFieldElem': _field, 'tools': tools, 'tools_web': tools_web, 'curObject': taskDoc.TopElem } ] );
...
function GetHierarchicalTasksPerson(iPersonID, iProjectID) {
	...
	function getTaskResult(id, task, executors, parent_id) {
		oTaskRes = { id: id, name: task.name.Value, fullname: i18n.t( 'netotvet' ), position: '', date_start: '', date_end: '', status: '', parent_id: OptInt(parent_id, '') }
